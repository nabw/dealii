...
namespace PETScWrappers
{
  namespace MPI
  {
                            ...
                            const MPI_Comm &           communicator)
    {
      ...
      do_reinit_IS(local_rows, local_active_rows, local_columns, local_active_columns, sparsity_pattern);
    }
...
#  endif
      ...
      ISCreateGeneral(communicator, n_l2g_row, idx_glob_row, PETSC_COPY_VALUES,
                          &is_glob_row);
      ...
      ISCreateGeneral(communicator, n_l2g_col, idx_glob_col, PETSC_COPY_VALUES,
                          &is_glob_col);
      ...
      MatSetType(local_matrix, MATSEQAIJ); // SEQ as it is local! TODO: Allow for OpenMP parallelization in local node.
      if (local_rows.n_elements() > 0)
        {
            ...
            for (PetscInt i = local_row_start; i < local_row_end; ++i)
              {
                ...
                const PetscInt row_length = sparsity_pattern.row_length(global_row_index);
                rowstart_in_window[i + 1 - local_row_start] =
                  rowstart_in_window[i - local_row_start] + row_length;
                n_cols += row_length;
              }
          ...
          // now copy over the information
          // from the sparsity pattern. For this we first invert the column index set.
