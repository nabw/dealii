...
template <int dim> class LaplaceProblem {
public:
  LaplaceProblem();
...
template <int dim> void LaplaceProblem<dim>::setup_system() {
  ...
  system_matrix.reinit_IS(locally_owned_dofs, locally_active_dofs, locally_owned_dofs,
                       locally_active_dofs, dsp,
                       mpi_communicator);
}
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i) {
          for (unsigned int j = 0; j < dofs_per_cell; ++j)
            cell_matrix(i, j) += (fe_values.shape_grad(i, q_point) *
                                      fe_values.shape_grad(j, q_point) * std::pow(cell->diameter(), 2) +
...
template <int dim> void LaplaceProblem<dim>::solve() {
...
template <int dim> void LaplaceProblem<dim>::refine_grid() {
...
void LaplaceProblem<dim>::output_results() const {
  ...
  data_out.write_vtu_with_pvtu_record("./", "solution", 0, mpi_communicator,
                                      2, 8);
}

// @sect4{LaplaceProblem::run}
template <int dim> void LaplaceProblem<dim>::run() {
  pcout << "Running"
